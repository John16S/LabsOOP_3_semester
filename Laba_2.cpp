...
public:
	...
	instrumenti_PO& copy(const instrumenti_PO& po); // constuctor kopirovaniye 
...
instrumenti_PO& instrumenti_PO::copy(const instrumenti_PO& po) { //metod kopirovaniye poley odnogo objecta v drugoy
	if (this != &po) {
		if (official_name != NULL) delete[] official_name;
		if (po.official_name != NULL) {
			official_name = new char[strlen(po.official_name) + 1];
